...
(function () {
    ...
    function showNotification(message, type = 'info') {
        ...
        notification.style.cssText = `
            ...
            border-radius: 8px;
            z-index: 1001;
            background-color: ${type === 'success' ? '#e1f2ec' : 'rgba(100, 116, 139, 0.12)'};
            color: ${type === 'success' ? '#0d9488' : 'var(--neutral-text)'};
        `;
...
class PhraseTraining {
    ...
    createTrainingModal(selectedPhrasesCount) {
        ...
        modal.innerHTML = `
            ...
            <div style="background: var(--bg-color); width: 95vw; height: 90vh; border-radius: 12px; padding: 24px; margin: 16px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
                ...
                <div style="text-align: center; max-width: 400px;">
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                        <div style="position: relative; text-align: center;">
                            ...
                            <h2 style="font-size: 30px; font-weight: 700; color: var(--body-1); margin-top: 45px;">Phrase Training</h2>
                        </div>
                        <p style="color: var(--body-2); margin-top: 8px; font-size: 16px;">Выбрано фраз: ${selectedPhraseCount}</p>
                    </div>
                    
                    <div style="margin-top: 64px;">
                        <p style="color: var(--body-2); margin-bottom: 24px; font-size: 16px;">Выберите количество фраз</p>
                        ...
                        <div class="phrase-count-controls" style="display: flex; align-items: center; justify-content: center; gap: 24px; margin-bottom: 64px;">
                            ...
                            <span class="phrase-count" style="font-size: 24px; color: var(--body-1);">${selectedPhrasesCount}</span>
                            <button class="plus-btn control-btn">+</button>
                        </div>

                        <button class="settings-btn" style="background: none; border: none; cursor: pointer; color: var(--body-2); font-size: 18px; display: flex; align-items: center; gap: 8px; margin: 0 auto 64px;">
    ...
    createModesModal() {
        ...
        modesModal.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                <h2 style="color: var(--body-1); font-size: 24px;">Режимы тренировки</h2>
                <button class="close-modes-modal" style="color: var(--body-2); background: none; border: none; font-size: 32px; cursor: pointer; padding: 8px;">×</button>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr; gap: 16px; max-width: 600px; margin: 0 auto;">
                ${Object.keys(this.trainingModes).map(mode => `
                    <label class="mode-item ${this.trainingModes[mode] ? 'active' : ''}" data-mode="${mode}">
                        ...
                        <div>
                            <h3 style="color: var(--body-1); font-size: 18px; margin: 0 0 4px 0;">${this.getModeTitle(mode)}</h3>
                            <p style="color: var(--body-2); font-size: 14px; margin: 0;">${modeDescriptions[mode]}</p>
                        </div>
                    </label>
                `).join('')}
            </div>
            ...
            <div style="display: flex; justify-content: center; gap: 16px; margin-top: 32px;">
                <button class="cancel-modes-btn" style="background: var(--card-bg); color: var(--body-1); padding: 12px 32px; border-radius: 8px; border: none; cursor: pointer; transition: background-color 0.3s ease; font-size: 16px;">
                    Отмена
                </button>
                <button class="save-modes-btn" style="background: var(--accent-color); color: var(--body-1); padding: 12px 32px; border-radius: 8px; border: none; cursor: pointer; transition: background-color 0.3s ease; font-size: 16px;">
                    Сохранить
                </button>
            </div>
        `;
        ...
        return `
            <div style="margin-bottom: 24px; width: 100%;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="color: var(--body-2); font-size: 14px;">${this.getModeTitle(mode)}</span>
                    <span style="color: var(--body-2); font-size: 14px;">${current}/${total}</span>
                </div>
                <div style="width: 100%; height: 4px; background: var(--card-bg); border-radius: 2px; overflow: hidden;">
                    <div style="width: ${(current / total) * 100}%; height: 100%; background: var(--accent-color); border-radius: 2px;"></div>
                </div>
            </div>
        `;
    }
    ...
    showCompletionModal() {
        ...
        modal.innerHTML = `
            <div style="background: var(--bg-color); width: 95vw; max-width: 500px; border-radius: 12px; padding: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 16px;">Все режимы тренировки завершены!</h2>
                <p style="color: var(--body-2); font-size: 18px; margin-bottom: 16px;">
                    Отличная работа! Вы успешно завершили все выбранные режимы тренировки.
                </p><p style="color: var(--body-2); font-size: 18px; margin-bottom: 32px;">
                    Общее количество правильных ответов: ${this.totalCorrectAnswers} из ${totalPossibleAnswers}
                </p>
                <button class="close-completion" style="background: var(--accent-color); color: var(--body-1); padding: 12px 32px; border-radius: 8px; border: none; cursor: pointer; transition: background-color 0.3s ease;">
                Закрыть
                </button>
            </div>
        `;
...
PhraseTraining.prototype.showCompletionModal = function () {
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 95vw; max-width: 500px; border-radius: 12px; padding: 32px; text-align: center;">
            ...
            <p style="color: var(--body-2); font-size: 18px; margin-bottom: ${autoMovedCount > 0 ? '8px' : '32px'};">
                Общее количество правильных ответов: ${this.totalCorrectAnswers} из ${totalPossibleAnswers}
...
function showNotification(message, type = 'info') {
    ...
    notification.style.cssText = `
        ...
        border-radius: 8px;
        z-index: 1001;
        background-color: ${type === 'success' ? '#e1f2ec' :
...
PhraseTraining.prototype.createMatchPicturesModal = function () {
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 900px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            <button class="close-modal" style="position: absolute; top: 16px; right: 16px; color: var(--body-2); background: none; border: none; font-size: 24px; cursor: pointer;">×</button>
            
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">Match Pictures to Phrases</h2>
                <p style="color: var(--body-2);">Select the image that best matches the phrase</p>
                <p style="color: var(--body-2);">Выберите изображение, которое лучше всего соответствует фразе</p>
            </div>
    ...
    if (this.currentMatchPicturesIndex >= this.matchPicturesData.length) {
        ...
        contentArea.innerHTML = `
            <div style="text-align: center; padding: 32px;">
                <h2 style="color: var(--body-1); font-size: 28px; margin-bottom: 16px;">Training Complete!</h2>
                <p style="color: var(--body-2); font-size: 18px; margin-bottom: 24px;">
                    You correctly matched ${this.matchPicturesCorrectCount} out of ${this.matchPicturesData.length} phrases.
                </p>
                <button class="continue-training-btn" style="background: var(--accent-color); color: var(--body-1); padding: 12px 32px; border-radius: 8px; border: none; cursor: pointer; font-size: 16px;">
    ...
    try {
        ...
        contentArea.innerHTML = `
            <div style="margin-bottom: 24px;">
                <div class="phrase-to-match" style="background: var(--card-bg); border-radius: 8px; padding: 16px; margin-bottom: 16px;">
                    <h3 style="color: var(--body-1); font-size: 20px; margin: 0 0 8px 0;">${currentPhrase.text}</h3>
                    <p style="color: var(--body-2); margin: 0;">${currentPhrase.translation}</p>
                    <p style="color: var(--body-2); font-style: italic; margin: 8px 0 0 0;">${currentPhrase.example}</p>
                </div>
            ...
            <div style="display: flex; justify-content: center; margin-top: 32px;">
                <button class="next-question-btn" style="background: var(--accent-color); color: var(--body-1); padding: 12px 32px; border-radius: 8px; border: none; cursor: pointer; opacity: 0; pointer-events: none; transition: opacity 0.3s ease; font-size: 16px;">
                    Next Question
                </button>
            </div>
        `;
        ...
        contentArea.innerHTML = `
            <div style="color: #f87171; background: var(--danger-bg); padding: 20px; border-radius: 8px; text-align: center;">
                ...
                <button class="retry-btn" style="background: var(--accent-color); color: var(--body-1); padding: 8px 24px; border-radius: 8px; border: none; cursor: pointer; margin-top: 16px;">
                    Continue to Next Training
                </button>
            </div>
        `;
...
PhraseTraining.prototype.createWritePhrasalVerbModal = function () {
    ...
    // Начальная HTML-структура
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 700px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            <button class="close-modal" style="position: absolute; top: 16px; right: 16px; color: var(--body-2); background: none; border: none; font-size: 24px; cursor: pointer;">×</button>
            
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">Write the Correct Phrasal Verb</h2>
                <p style="color: var(--body-2);">Type the phrasal verb that matches the definition</p>
            </div>
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 700px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            ...
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">Write the Correct Phrasal Verb</h2>
                <p style="color: var(--body-2);">Type the phrasal verb that matches the definition</p>
                <p style="color: var(--body-2);">Введите фразовый глагол, соответствующий определению.</p>
            </div>
...
PhraseTraining.prototype.showNextWritePhrasalVerbQuestion = function (modal) {
    ...
    try {
        ...
        // Генерируем контент для текущего вопроса
        contentArea.innerHTML = `
            <div style="margin-bottom: 32px; background: var(--card-bg); border-radius: 8px; padding: 20px;">
                <h3 style="color: var(--body-1); font-size: 20px; margin: 0 0 16px 0;">Definition:</h3>
                <p style="color: var(--body-2); font-size: 18px; margin: 0;">${currentPhrase.translation}</p>
            </div>
            
            <div class="answer-area" style="margin-bottom: 32px;">
                <label for="phrasal-verb-answer" style="display: block; color: var(--body-1); margin-bottom: 8px; font-size: 16px;">Enter the phrasal verb:</label>
                <div style="display: flex; gap: 16px;">
                    <input type="text" id="phrasal-verb-answer" style="flex: 1; padding: 12px 16px; background: var(--card-bg); border: 1px solid var(--card-stroke); border-radius: 8px; color: var(--body-1); font-size: 16px;" placeholder="Type the phrasal verb here...">
                    <button class="check-answer-btn" style="background: var(--accent-color); color: var(--body-1); padding: 0 24px; border-radius: 8px; border: none; cursor: pointer; font-size: 16px;">
                        Check
                    </button>
                </div>
            </div>
            
            <div class="feedback-area" style="margin-bottom: 32px; min-height: 60px; display: none; background: var(--card-bg); border-radius: 8px; padding: 20px;">
            ...
            checkButton.addEventListener('click', () => {
                ...
                if (isCorrect) {
                    if (feedbackMessage) {
                        feedbackMessage.innerHTML = `
                            ...
                            <p style="color: var(--body-1); margin-top: 8px;">${correctAnswer}</p>
                        `;
                    }
                    if (feedbackArea) {
                    ...
                    if (feedbackMessage) {
                        feedbackMessage.innerHTML = `
                            ...
                            <p style="color: var(--body-1); margin-top: 8px;">The correct answer is: ${correctAnswer}</p>
                        `;
                    }
                    if (feedbackArea) {
                ...
                if (currentPhrase.example && exampleContainer) {
                    ...
                    exampleContainer.innerHTML = `
                        <p style="color: var(--body-2); margin-top: 16px; font-size: 16px;">Example:</p>
                        <div style="background: var(--card-bg); padding: 16px; border-radius: 8px; margin-top: 8px; display: inline-block;">
                            <p style="color: var(--body-1); margin: 0;">${exampleWithHighlight}</p>
                        </div>
                    `;
...
PhraseTraining.prototype.createFillGapsModal = function () {
    ...
    if (preparedData.length === 0) {
        ...
        modal.innerHTML = `
            <div style="background: var(--bg-color); width: 90vw; max-width: 700px; border-radius: 12px; padding: 24px; position: relative; text-align: center;">
                <button class="close-modal" style="position: absolute; top: 16px; right: 16px; color: var(--body-2); background: none; border: none; font-size: 24px; cursor: pointer;">×</button>
                
                <div style="margin: 32px 0;">
                    <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 16px;">No Valid Examples Found</h2>
                    <p style="color: var(--body-2); margin-bottom: 24px;">
                        This training mode requires examples in data-extra-examples attributes. 
                        No phrases with examples were found.
                    </p>
                    <button class="continue-btn" style="background: var(--accent-color); color: var(--body-1); padding: 12px 32px; border-radius: 8px; border: none; cursor: pointer; font-size: 16px;">
                        Continue
                    </button>
                </div>
            </div>
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 700px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            ...
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">Fill in the Gaps</h2>
                <p style="color: var(--body-2);">Choose the correct phrasal verb to complete each sentence</p>
                <p style="color: var(--body-2);">Выберите правильный фразовый глагол, чтобы закончить каждое предложение.</p>
            </div>
            
            <div style="margin-bottom: 24px; width: 100%;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="color: var(--body-2); font-size: 14px;">Fill in the Gaps with the Correct Phrasal Verb</span>
                    <span style="color: var(--body-2); font-size: 14px;" class="progress-indicator-label">1/${preparedData.length}</span>
                </div>
                <div style="width: 100%; height: 4px; background: var(--card-bg); border-radius: 2px; overflow: hidden;">
                    <div class="progress-indicator-fill" style="width: ${(1 / preparedData.length) * 100}%; height: 100%; background: var(--accent-color); border-radius: 2px;"></div>
                </div>
            </div>
    ...
    try {
        ...
        contentArea.innerHTML = `
            <div style="margin-bottom: 32px;">
                <div class="sentence-container" style="background: var(--card-bg); border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                    <p style="color: var(--body-1); font-size: 18px; line-height: 1.6; margin: 0;" id="gapped-sentence">${exampleSentence.processedSentence}</p>
                </div>
                
                <div class="options-container" style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 24px;">
                    ${shuffledOptions.map((option, index) => `
                        <button class="option-btn" data-text="${option.text}" data-correct="${option.isCorrect}" style="background: var(--card-bg); border: 1px solid var(--card-stroke); color: var(--body-1); padding: 12px 16px; border-radius: 8px; text-align: left; cursor: pointer; font-size: 16px; transition: all 0.3s;">
                            ${option.text}
                        </button>
                    `).join('')}
                </div>
...
PhraseTraining.prototype.createChooseCorrectMeaningModal = function () {
    ...
    // Начальная HTML-структура
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 800px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            <button class="close-modal" style="position: absolute; top: 16px; right: 16px; color: var(--body-2); background: none; border: none; font-size: 24px; cursor: pointer;">×</button>
            
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">Choose the Correct Meaning</h2>
                <p style="color: var(--body-2);">Read the statement and select the meaning of the highlighted phrasal verb</p>
                <p style="color: var(--body-2);">Прочитайте утверждение и выберите значение выделенного фразового глагола.</p>
            </div>
...
PhraseTraining.prototype.showNextChooseMeaningQuestion = function (modal) {
    ...
    try {
        ...
        contentArea.innerHTML = `
            <div style="margin-bottom: 32px;">
                <div class="statement-container" style="background: var(--card-bg); border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                    <p style="color: var(--body-1); font-size: 18px; line-height: 1.6; margin: 0;">${exampleWithHighlight}</p>
                </div>
                
                <div class="feedback-message" style="min-height: 50px; text-align: center; margin-bottom: 16px;">
                    <p style="color: var(--body-2);">What does the highlighted phrasal verb mean?</p>
                </div>
                
                <div class="meaning-options" style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 24px;">
                    ${options.map((option, index) => `
                        <button class="meaning-option" data-correct="${option.isCorrect}" style="background: var(--card-bg); border: 1px solid var(--card-stroke); color: var(--body-1); padding: 16px; border-radius: 8px; text-align: left; cursor: pointer; font-size: 16px; transition: all 0.3s;">
                            ${option.text}
                        </button>
                    `).join('')}
                </div>
...
PhraseTraining.prototype.createTrueOrFalseModal = function () {
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 800px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            ...
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">True or False?</h2>
                <p style="color: var(--body-2); margin-bottom: 6px;">Is the phrasal verb used correctly in this sentence?</p>
                <p style="color: var(--body-2);">Правильно ли использован фразовый глагол в этом предложении?</p>
            </div>
...
PhraseTraining.prototype.showNextTrueFalseQuestion = function (modal) {
    ...
    try {
        ...
        contentArea.innerHTML = `
            <div style="margin-bottom: 32px;">
                <div class="sentence-container" style="background: var(--card-bg); border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                    <p style="color: var(--body-1); font-size: 18px; line-height: 1.6; margin: 0;">${currentQuestion.processedSentence}</p>
                </div>
                
                <div class="true-false-buttons" style="display: flex; justify-content: center; gap: 32px; margin-bottom: 32px;">
                    <button class="true-btn" data-value="true" style="background: var(--card-bg); color: var(--body-1); border: 2px solid var(--card-stroke); border-radius: 8px; padding: 16px 48px; font-size: 18px; font-weight: 600; cursor: pointer; transition: all 0.3s ease;">
                        TRUE
                    </button>
                    <button class="false-btn" data-value="false" style="background: var(--card-bg); color: var(--body-1); border: 2px solid var(--card-stroke); border-radius: 8px; padding: 16px 48px; font-size: 18px; font-weight: 600; cursor: pointer; transition: all 0.3s ease;">
                        FALSE
                    </button>
                </div>
                ...
                <div class="correct-sentence" style="display: none; background: var(--card-bg); border-radius: 8px; padding: 20px; margin-top: 16px; margin-bottom: 24px;">
                    <p style="color: var(--correct-text); font-size: 14px; margin: 0 0 8px 0;">Correct sentence:</p>
                    <p style="color: var(--body-1); font-size: 18px; line-height: 1.6; margin: 0;">${correctSentenceHighlighted}</p>
                </div>
            </div>
...
PhraseTraining.prototype.createFindMistakeModal = function () {
    ...
    modal.innerHTML = `
        <div style="background: var(--bg-color); width: 90vw; max-width: 800px; height: 90vh; border-radius: 12px; padding: 24px; overflow-y: auto; position: relative;">
            ...
            <div style="margin-bottom: 32px; text-align: center;">
                <h2 style="color: var(--body-1); font-size: 24px; margin-bottom: 8px;">Find the Mistake</h2>
                <p style="color: var(--body-2); margin-bottom: 6px;">Correct the highlighted phrasal verb in the sentence below</p>
                <p style="color: var(--body-2);">Исправьте выделенный фразовый глагол в предложении ниже</p>
            </div>
...
PhraseTraining.prototype.showNextFindMistakeQuestion = function (modal) {
    ...
    try {
        ...
        contentArea.innerHTML = `
            <div style="margin-bottom: 32px;">
                <div class="sentence-container" style="background: var(--card-bg); border-radius: 8px; padding: 20px; margin-bottom: 24px;">
                    <p style="color: var(--body-1); font-size: 18px; line-height: 1.6; margin: 0;">${currentQuestion.processedExample}</p>
                </div>
                
                <div class="correction-container" style="margin-bottom: 24px;">
                    <label for="correction-input" style="display: block; color: var(--body-1); margin-bottom: 8px; font-size: 16px;">
                        Correct the sentence <span style="color: var(--body-2); font-size: 14px;">(you can write just the correct phrasal verb or the entire sentence)</span>:
                    </label>
                    <div style="display: flex; gap: 12px; align-items: center;">
                        <input type="text" id="correction-input" style="flex-grow: 1; padding: 12px 16px; background: var(--card-bg); border: 1px solid var(--card-stroke); border-radius: 8px; color: var(--body-1); font-size: 16px;" 
                            placeholder="Type the corrected phrasal verb or full sentence...">
                        <button class="check-btn" style="background: var(--accent-color); color: var(--body-1); padding: 12px 24px; height: 48px; border-radius: 8px; border: none; cursor: pointer; font-size: 16px; white-space: nowrap;">
                            Check
                        </button>
                    </div>
                </div>
                ...
                <div class="hint-container" style="text-align: center; margin-bottom: 16px; display: none;">
                    <button class="hint-btn" style="background: none; border: none; color: var(--body-2); text-decoration: underline; cursor: pointer; font-size: 14px;">
                        Show Hint
                    </button>
                </div>
            </div>
